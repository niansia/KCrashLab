...
Reviewer-facing experiment summaries include provenance. The definition digest binds all controlled parameters; `GIT_TREE_SHA256_V1` reads path, mode, and blob content from the pinned commit in Git's object database, covering source, tests, schemas, samples, CI, and documentation while deliberately excluding generated results to avoid a circular hash. It never hashes platform-dependent checkout bytes. Under `SOURCE_COMMIT_TIME`, the CLI resolves `HEAD`, requires a clean worktree, and checks that `git_commit == HEAD` before and after provenance construction. Exact artifact bytes remain protected by each result manifest, and Git text conversion is disabled below `results/recorded` so checkout cannot rewrite the hashed evidence bytes.

Canonical M1 simulation evidence records only semantics that can affect the deterministic backend: the versioned backend contract, simulator version, virtual epoch, and canonical scenario-fixture digest. Host operating-system, Hyper-V, SDK, and WDK observations are intentionally excluded from the byte-identical simulation manifest; those belong to the separate host capability probe and do not alter the synthetic result.
